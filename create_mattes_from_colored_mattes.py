...
def populate_char_to_color_dict():
    """From the csv file mapping XMem2 matte colors to shot elements, create a dictionary with the elements as keys. Note the BGR colors for OpenCV."""
    # TODO: change the name of the google sheet to matte instead of cutout   
    csv_file_path = r'X:\project\csv_files\xmem_cutout_map.csv'
...
def reverse_lookup(dictionary, value):
    """Find the dictionary key for the given value."""
    for key, val in dictionary.items():
        if val == value:
            return key
    return None  # Return None if the value is not found in any key-value pair
...
def get_frame_range(folder_path):
    ...
    # looks for a sequence of digits (\d+) followed by ".png" and ensures that it is a whole word using word boundaries (\b)
...
parser.add_argument('elements', type=str, help='Comma separated (no spaces, no brackets) list of elements that need black and white mattes. Example: element1,element2')
...
# Get the frame range from the denoise path
denoise_path = get_denoise_path(args.colormatte_path)

if args.start_frame == None or args.end_frame == None:
