...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False, manually_curated_masks=False, disable_memory_updates=False, do_not_add_mask_to_memory=False, holistic_features=None):
        ...
        if manually_curated_masks:
            ...
            is_mem_frame = ((self.curr_ti-self.last_mem_ti >=
                            self.mem_every) or (mask is not None)) and (not end)
        ...
        is_deep_update = (
            ...
            (not self.deep_update_sync and self.curr_ti - \
             self.last_deep_update_ti >= self.deep_update_every)  # no-sync
        ) and (not end)
        is_normal_update = (
            not self.deep_update_sync or not is_deep_update) and (not end)

        # if deep_update_sync is enabled, then it's either normal_update (for non-mem frames) or deep_update (for mem frames)
        # if deep_update_sync is disabled, then it's normal_update (for non-mem frames) and BOTH (for mem frames)
        # So if deep_update_sync is enabled, then it's either HiddenUpdater or HiddenReinforcer applied to hidden state
        ...
        if need_segment:
            ...
            if is_normal_update:  # else hidden would be == None
        ...
        if mask is not None:
            # literally just pad with 0s so that the shape is 16-divisible
            ...
            if pred_prob_no_bg is not None:
                ...
                if valid_labels is not None:
                    shift_by_one_non_labels = [i for i in range(
                        pred_prob_no_bg.shape[0]) if (i+1) not in valid_labels]
            ...
            # aggregating predictions across multiple objects/classes
