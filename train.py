...
# To allow calling multiple networks with torch/cuda 
        ...
        try:
            while total_iter < config['iterations'] + config['finetune']:
                ...
                for data in train_loader:
                    ...
                    of_model = build_model()
                    for j in range(0, data['rgb'].shape[0]): #poner que se hace por cada uno del batch y ver si sirve y a entrenar 
                        for i in range(1, data['rgb'].shape[1]): 
                            image1, image2 = data['rgb'][j][i-1], data['rgb'][j][i]
                            # image1, image2 = Variable(image1, requires_grad=False), Variable(image2, requires_grad=False) 
                            # image1, image2 = image1.detach().numpy(),  image2.detach().numpy() 
                            flow = compute_flow(of_model, image1, image2) #es el flow de imagen i porque siempre es frame anterior y el presente
                            flow_t = torch.Tensor(flow)
                            data['optical_flow'][j][i] = flow_t
                    of_model = None
